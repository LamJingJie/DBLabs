...
public class LockManager {
    ...
    public synchronized void acquireLock(PageId pageId, TransactionId tid, Permissions perm) throws TransactionAbortedException {
    ...
    private boolean grantLock(TransactionId tid, PageId pageId, Permissions perm){
        ...
        if(pageLocks == null || pageLocks.isEmpty()) return true;

        // if only this transaction holds the lock
        if(pageLocks.size() == 1 && pageLocks.containsKey(tid)){
            // Allow reacquiring, upgrading (read -> write) or downgrading (wrte -> read) the lock
            return true;
        }
        ...
        if(perm == Permissions.READ_ONLY){
            for(Permissions p : pageLocks.values()){
                if(p == Permissions.READ_WRITE) return false;
            }
            return true;
        }

        // exclusive lock: only if no other locks or if transaction is the only one holding a shared lock on that object
    ...
    public synchronized void releaseLock(PageId pageId, TransactionId tid){
        ...
        if(pageLocks != null) {
            ...
            if(pageLocks.isEmpty()) locks.remove(pageId);
